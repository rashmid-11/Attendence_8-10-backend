...
app.get('/api/degree/:empID', async (req, res) => {
    const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
...
app.get('/api/yearss/:empID', async (req, res) => {
    const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
        if (isNaN(empID)) {
            return res.status(400).send('Invalid EmpID');
        }
...
app.get('/api/semester/:empID', async (req, res) => {
    const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
        if (isNaN(empID)) {
            return res.status(400).send('Invalid EmpID');
        }
...
// Get all Subjects
app.get('/api/subject/:empID', async (req, res) => {
    const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
        if (isNaN(empID)) {
            return res.status(400).send('Invalid EmpID');
        }
...
app.post('/api/prefixes', async (req, res) => {
  ...
  app.get('/api/prefixes/:empID', async (req, res) => {
    const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
        if (isNaN(empID)) {
            return res.status(400).send('Invalid EmpID');
        }
...
app.get('/api/phases/:empID', async (req, res) => {
    const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
        if (isNaN(empID)) {
            return res.status(400).send('Invalid EmpID');
        }
...
// Get all premises
app.get('/api/premises/:empID', async (req, res) => {
  const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
  if (isNaN(empID)) {
      return res.status(400).send('Invalid EmpID');
  }
...
app.get('/api/onepremises/:id', async (req, res) => {
  try {
      let { id } = req.params;
...
app.put('/api/onepremises/:id', async (req, res) => {
  const { id } = req.params;
  const {
      Department,
      depid,
...
app.put('/api/premises/update/status/:id', async (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
...
//////////////////////Student///////////////////////////////////////////
  ...
  app.post("/api/professor", async (req, res) => {
    const { empId, firstName, middleName, lastName, email, contact, birthDate, joiningDate, namePrefix, department, education, post, isActive, designation } = req.body;
    ...
    try {
        ...
        await request.query(`
            ...
            (EmpNumber, FirstName, MiddleName, LastName, Email, Phone, BirthDate, JoinDate, Prefix, Department, Education, Post, Status, Designation, CreateDate) 
            VALUES 
            (@EmpId, @FirstName, @MiddleName, @LastName, @Email, @Phone, @BirthDate, @JoinDate, @Prefix, @Department, @Education, @Post, @Status, @Designation, @CreatedDate)
        `);
...
app.get('/api/professors/:empID', async (req, res) => {
    const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
        if (isNaN(empID)) {
            return res.status(400).send('Invalid EmpID');
        }
     try {
            ...
            // Parameterized query to prevent SQL injection
            const result = await sql.query`
                SELECT * FROM Tbl_Professor WHERE EmpNumber = ${empID}`;
...
app.get('/api/getstudents/:empID', async (req, res) => {
  const empID = parseInt(req.params.empID); // Ensure EmpID is an integer
  if (isNaN(empID)) {
      return res.status(400).send('Invalid EmpID');
  }
