...
app.post('/api/loginadmin', async (req, res) => {
  ...
  try {
      const result = await pool
          ...
          .query('SELECT ID, EmailID, UserName, DeptName FROM [dbo].[Tbl_Admin] WHERE EmailID = @EmailID AND Password = @Password');
...
app.post('/api/logout', (req, res) => {
  ...
  // Verify token and clear session logic
  if (!token) {
  ...
  jwt.verify(token, 'your_secret_key', (err, decoded) => {
    ...
    // Clear the user session or token in your database
    const user = users.find(u => u.username === decoded.username); // Find user by decoded username
    if (user) {
      user.token = ''; // Clear the token or implement any other session clearing logic
    }
...
app.put('/api/degrees/update/:id', async (req, res) => {
  const { id } = req.params;
  const { Status } = req.body;
...
app.put('/api/years/update/:id', async (req, res) => {
  const { id } = req.params;
  const { Status } = req.body;
...
app.put('/api/semesters/update/:id', async (req, res) => {
  const { id } = req.params;
  const { Status } = req.body;
...
app.put('/api/subjects/update/:id', async (req, res) => {
  const { id } = req.params;
  const { Status } = req.body;
...
app.put('/api/prefixes/update/:id', async (req, res) => {
  const { id } = req.params;
  const { Status } = req.body;
...
app.post('/api/premises', async (req, res) => {
  const { Department, lectureType, professor, classroom, status, startTime, endTime, additionalInfo, selectedDate, createdDate, updatedDate } = req.body;

  try {
      // Connect to the database
      await sql.connect(config);

      // Insert the new premises into the database and get the inserted ID
      const result = await sql.query`
          INSERT INTO Tbl_Premises (Department, LectureType, Professor, Classroom, Status, StartTime, EndTime, AdditionalInfo, SelectedDate, CreatedDate, UpdatedDate)
          OUTPUT INSERTED.ID
          VALUES (${Department}, ${lectureType}, ${professor}, ${classroom}, ${status}, ${startTime}, ${endTime}, ${additionalInfo}, ${selectedDate}, ${createdDate}, ${updatedDate})`;
...
app.get('/api/premises', async (req, res) => {
  try {
      ...
      console.error('SQL error:', error);
      res.status(500).send('Error retrieving premises');
  }
});
...
app.get('/api/premises/:id', async (req, res) => {
  try {
    let { id } = req.params;
    id = id.replace(':', ''); // Remove any leading colons
...
// Update an existing premises
app.put('/api/premises/:id', async (req, res) => {
  const { id } = req.params;
  const { Department, lectureType, professor, classroom, status, startTime, endTime, additionalInfo, selectedDate, updatedDate } = req.body;
  ...
  try {
      ...
      res.status(200).send({ message: 'Premises updated successfully' });
  } catch (error) {
      console.error(error);
      res.status(500).send('Internal Server Error');
  }
...
app.put('/api/premises/update/:id', async (req, res) => {
  const { id } = req.params;
  const { Status } = req.body; // Use the same key as in the frontend

  try {
      await sql.connect(config);

      // Check if Status is defined and not null before updating
      if (typeof Status === 'undefined' || Status === null) {
          return res.status(400).send('Invalid Status value');
      }

      await sql.query`UPDATE Tbl_Premises SET Status = ${Status} WHERE ID = ${id}`;
      res.status(200).send('Status updated successfully');
  } catch (error) {
      console.error(error);
      res.status(500).send('Error updating status');
...
// Delete a premises
app.delete('/api/premises/delete/:id', async (req, res) => {
  const id = req.params.id;
